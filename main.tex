...
\end{figure}
\section{Case study}
We evaluated this meta-model against the example of the privacy policy of real-world banks. To demonstrated how flexible and clearly to use it is, we used a meta-model to described its usual business processes. Since we did not implement DSL, because we do not want to use either graphic or text notation, which will be discussed later, we used the existing xmi for verification, which is available in the development environment. To cover all the first 50 members of the GDPR, we made several use-cases. To simplify the example, the bank employs two workers: Ned Flanders and Patti Bouvier. \newline The first group of use cases deals with the opening of an account, which is considered a basic operation in the system since it collects information about the client. During data collection, the client defines the purpose for which the use of his data is allowed. In this case, the user, Edna Karabapel, wants to use data only for out of scope purposes and sub reason personal activity. In order to collect its data, it is necessary to submit Consent. After that, Edna checked the status of the bank account. The bank officer accessed her personal data, so we tested Art 9. This Art checks whether the purpose of data processing has been previously defined. In this way, we also tested Art 9, which explicitly states that it is used for each use of biomedical seas or predefines a set of goals, as well as for each data or must be used for public interest purposes. To start the transfer, it is necessary to specify the transfer certificate, it is also necessary that this document is valid. After the data transfer, it is necessary to create a notification on the successful transfer of personal data. \newline The second group of use cases deals with the collection and processing of juvenile data.
\begin{figure}[H]
\centering
...
\end{minipage}
\end{figure}
In this way, we also tested Art. 9, which explicitly states that if the use of biomedical data is not previously defined, then its use for purposes of public interest is allowed.
\begin{figure}[H]
\centering
\begin{minipage}{.5\textwidth}
  \centering
